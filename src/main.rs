...
fn fix_image(path: &Path, args: &Cli) {
    println!("Loading image from {}...", &path.display());
        ...
        (0..width * height)
            ...
            .filter_map(|index: u32| {
                ...
                let maybe_closest = [
                    ...
                    voronoi_read.get_closest((x + search_radius, y), args.edge_mode),
                    voronoi_read
                        .get_closest((x - search_radius, y - search_radius), args.edge_mode),
                    voronoi_read
                        .get_closest((x - search_radius, y + search_radius), args.edge_mode),
                    voronoi_read
                        .get_closest((x + search_radius, y - search_radius), args.edge_mode),
            ...
            .collect::<Vec<_>>()
            .into_iter() // serialise
            .for_each(|(xy_unsigned, new_closest)| {
                voronoi_write.set_closest(xy_unsigned, new_closest)
            });
        ...
        OutputAs::Bleed => {
            ...
            (0..width * height).for_each(|index: u32| {
                ...
                if let Some(position) =
                    voronoi_read.get_closest((x as i64, y as i64), args.edge_mode)
                {
    ...
    let output = args.output.as_deref().unwrap_or(path);
